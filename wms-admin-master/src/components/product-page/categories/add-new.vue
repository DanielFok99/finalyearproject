    ...
    <form class="validate-form" @submit.prevent="submit">
      <ModalHeader>
        <h2 class="font-medium text-base mr-auto">
          {{ props.category == null ? "Add New Category" : "Edit Category" }}
        </h2>
      </ModalHeader>
      <ModalBody class="grid grid-cols-12 gap-4 gap-y-3">
        ...
        <div class="col-span-12 sm:col-span-12">
          ...
          <input id="name" v-model.trim="category.name" class="form-control" name="name" type="text"/>
      ...
      <ModalFooter>
        ...
        <button class="btn btn-primary w-20" type="submit">
          {{ props.category == null ? "Create" : "Edit" }}
        </button>
      </ModalFooter>
    </form>
  </Modal>
...
onUpdated(() => {
  if (props.category != null && props.isOpen) {
...
const validate = useVuelidate(rules, toRefs(category));
...
const submit = () => {
  ...
  if (!validate.value.$invalid) {
    ...
    if (props.category != null) {
      web.put('/categories/' + props.category.id, data).then((response) => {
      ...
      //Create New
      web.post('/categories', data).then((response) => {
        if (response.status === 201) {
          responseMessage.value = response.data;
          showSuccessToast();
          closeModal();
