<template>
  <TabPanel>
    <attribute-add-new v-model:attribute="attribute" v-model:is-open="modalOpen"
                       @modal-close="modalStatus"></attribute-add-new>
...
const modalStatus = (value) => {
  ...
  getData().then(()=>{
    if (value === false){
      tabulator.value.setData(attributes.value);
    }
  });
}

const getData = async () => {
  await web.get('/attributes').then((response) => {
