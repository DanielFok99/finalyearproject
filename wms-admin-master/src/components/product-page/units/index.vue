<template>
  <TabPanel>
    <unit-add-new v-model:is-open="modalOpen" v-model:unit="unit"
                  @modal-close="modalStatus"></unit-add-new>
...
import UnitAddNew from './add-new.vue';
...
const modalStatus = (value) => {
  ...
  getData().then(()=>{
    if (value === false){
      tabulator.value.setData(units.value);
    }
  });
}

const getData = async () => {
  await web.get('/units').then((response) => {
...
const editData = (data) => {
  ...
  // attribute.value = attributes.value[attributeId];
  ...
  tabulator.value = new Tabulator(tableRef.value, {
    ...
    columns: [
      ...
      },
      {
        ...
        download: false,
        // formatter(cell) {
        //   return `<div class="font-medium whitespace-nowrap">${cell.getData().short_name}</div>`;
        // },
      },
      {
        title: "DEFAULT",
        ...
        formatter(cell) {
          ...
          if (cell.getData().is_default){
            return `
                <div class="flex items-center justify-center text-success">
                    <CheckSquareIcon class="w-4 h-4 mr-2"/>Yes
                </div>`;
          }else{
            return `
                <div class="flex items-center justify-center text-danger">
                    <CheckSquareIcon class="w-4 h-4 mr-2"/>No
                </div>`;
          }
