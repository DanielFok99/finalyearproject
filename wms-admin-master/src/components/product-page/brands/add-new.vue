    ...
    <form class="validate-form" @submit.prevent="submit">
      <ModalHeader>
        <h2 class="font-medium text-base mr-auto">
          {{ props.brands == null ? "Add New Brand" : "Edit Brand" }}
        </h2>
      </ModalHeader>
      <ModalBody class="grid grid-cols-12 gap-4 gap-y-3">
        ...
        <div class="col-span-12 sm:col-span-12">
          ...
          <input id="name" v-model.trim="brand.name" class="form-control" name="name" type="text"/>
...
onUpdated(() => {
  if (props.brand != null && props.isOpen) {
...
const validate = useVuelidate(rules, toRefs(brand));
...
const submit = () => {
  ...
  if (!validate.value.$invalid) {
    ...
    // Update Data
    if (props.brand != null) {
      web.put('/brands/' + props.brand.id, data).then((response) => {
      ...
      //Create New
      web.post('/brands', data).then((response) => {
        if (response.status === 201) {
          responseMessage.value = response.data;
          showSuccessToast();
          closeModal();
