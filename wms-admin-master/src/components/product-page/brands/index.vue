<template>
  <TabPanel>
    <brand-add-new v-model:brand="brand" v-model:is-open="modalOpen"
                   @modal-close="modalStatus"></brand-add-new>
...
import BrandAddNew from "@/components/product-page/brands/add-new.vue";
...
const modalStatus = (value) => {
  ...
  getData().then(() => {
    if (value === false) {
      tabulator.value.setData(brands.value);
    }
  });
}

const getData = async () => {
  await web.get('/brands').then((response) => {
