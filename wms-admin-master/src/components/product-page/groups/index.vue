<template>
  <TabPanel>
    <group-add-new v-model:group="group" v-model:is-open="modalOpen"
                   @modal-close="modalStatus"></group-add-new>
...
import GroupAddNew from './add-new.vue';
...
const modalStatus = (value) => {
  ...
  getData().then(()=>{
    if (value === false){
      tabulator.value.setData(groups.value);
    }
  });
...
const getData = async () => {
  await web.get('/groups').then((response) => {
